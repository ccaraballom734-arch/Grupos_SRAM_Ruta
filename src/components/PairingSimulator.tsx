...
export default function PairingSimulator() {
  ...
  return (
    <div id="pairing-simulator" className="backdrop-blur-md bg-white/45 border border-white/60 p-6 rounded-3xl shadow-xl hover:shadow-2xl transition-all">
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-[10px] text-gray-400 font-bold tracking-widest uppercase mt-0.5">Secciones 4 y 5 • Interactivo</p>
        </div>
        <span className="material-symbols-outlined text-brand-red text-2xl">settings_input_antenna</span>
      </div>
      ...
      <div className="bg-gray-50 border border-gray-100 rounded-2xl p-4 mb-6">
        <h4 className="text-[11px] font-bold text-gray-700 uppercase tracking-widest mb-2 flex items-center gap-1">
          <span className="material-symbols-outlined text-xs text-brand-red">info</span> Paso Actual de Emparejamiento:
        </h4>
        <div className="text-xs font-medium text-gray-700 leading-relaxed">
          {pairingState === 'IDLE' && (
            <div className="flex items-center gap-2">
              <span className="w-5 h-5 rounded-full bg-brand-red text-white flex items-center justify-center font-bold text-[10px]">1</span>
          ...
          {(pairingState === 'RD_PAIRING' || pairingState === 'PAIRING_OTHERS') && (
            <div className="space-y-2">
              <div className="flex items-center gap-2">
                <span className="w-5 h-5 rounded-full bg-brand-red text-white flex items-center justify-center font-bold text-[10px]">2</span>
                <span>Haga clic en los botones de cambio, desviador o Blips inalámbricos para emparejarlos en el sistema AXS.</span>
              </div>
              <p className="text-[10px] text-brand-red font-bold uppercase pl-7">
          ...
          {pairingState === 'PAIRED_SUCCESS' && (
            <div className="flex items-center gap-2">
              <span className="w-5 h-5 rounded-full bg-green-600 text-white flex items-center justify-center font-bold text-[10px]">✓</span>
        ...
        {components.map((comp) => {
          ...
          return (
            <div 
              ...
              className={`border rounded-2xl p-3.5 flex flex-col justify-between h-28 cursor-pointer select-none transition-all ${
                comp.paired 
            ...
            >
              <div className="flex justify-between items-start">
                ...
                <div className="flex items-center gap-1.5">
                  <span className="text-[9px] text-gray-400 font-bold uppercase tracking-widest">LED</span>
              ...
              <div className="mt-2">
                <p className="text-xs font-bold text-gray-800 leading-tight">{comp.name}</p>
                {comp.paired && comp.pairingOrder && (
                  <p className="text-[9px] text-green-700 font-bold mt-1 uppercase tracking-wider">
                    Sincronizado #{comp.pairingOrder}
                  </p>
                )}
                {isMainRD && comp.paired && (
                  <p className="text-[9px] text-brand-red font-bold mt-1 uppercase tracking-wider animate-pulse">
                    Principal (Activo)
                  </p>
                )}
                {!comp.paired && canClick && (
                  <p className="text-[9px] text-brand-red font-semibold mt-1 uppercase tracking-wide">
      ...
      <div className="flex gap-3 mb-6">
        {(pairingState === 'RD_PAIRING' || pairingState === 'PAIRING_OTHERS') && (
          <button
            ...
            className="flex-1 py-2.5 bg-black text-white hover:bg-brand-red rounded-xl font-bold text-xs uppercase tracking-wider transition-colors shadow-sm"
          >
        ...
        {pairingState === 'PAIRED_SUCCESS' && (
          <button
            ...
            className="flex-1 py-2.5 bg-green-600 text-white hover:bg-green-700 rounded-xl font-bold text-xs uppercase tracking-wider transition-colors shadow-sm"
          >
        ...
        {pairingState !== 'IDLE' && pairingState !== 'PAIRED_SUCCESS' && (
          <button
            onClick={resetPairing}
            className="py-2.5 px-4 border border-gray-200 text-gray-600 hover:bg-gray-50 rounded-xl font-bold text-xs uppercase tracking-wider transition-colors"
          >
            Cancelar
          </button>
        )}
      ...
      <div className="bg-gray-900 rounded-2xl p-4 font-mono text-[10px] text-gray-300">
        <p className="text-gray-500 font-bold uppercase tracking-widest border-b border-gray-800 pb-1 mb-2">
          ...
          <AnimatePresence>
            {logs.map((log, index) => (
              <motion.div
                ...
                className="leading-relaxed border-l-2 border-gray-700 pl-2"
              >
                {log}
              </motion.div>
            ))}
